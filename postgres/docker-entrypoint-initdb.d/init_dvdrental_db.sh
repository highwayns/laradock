...
if [ "$DVDRENTAL_POSTGRES_INIT" == 'true' ]; then
	psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
		CREATE USER $DVDRENTAL_POSTGRES_USER WITH PASSWORD '$DVDRENTAL_POSTGRES_PASSWORD';
		CREATE DATABASE $DVDRENTAL_POSTGRES_DB;
		GRANT ALL PRIVILEGES ON DATABASE $DVDRENTAL_POSTGRES_DB TO $DVDRENTAL_POSTGRES_USER;
		ALTER ROLE $DVDRENTAL_POSTGRES_USER CREATEROLE SUPERUSER;
	EOSQL
	echo
fi
