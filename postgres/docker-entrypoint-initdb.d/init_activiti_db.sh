...
if [ "$ACTIVITI_POSTGRES_INIT" == 'true' ]; then
	psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
		CREATE USER $ACTIVITI_POSTGRES_USER WITH PASSWORD '$ACTIVITI_POSTGRES_PASSWORD';
		CREATE DATABASE $ACTIVITI_POSTGRES_DB;
		GRANT ALL PRIVILEGES ON DATABASE $ACTIVITI_POSTGRES_DB TO $ACTIVITI_POSTGRES_USER;
		ALTER ROLE $ACTIVITI_POSTGRES_USER CREATEROLE SUPERUSER;
	EOSQL
	echo
fi
