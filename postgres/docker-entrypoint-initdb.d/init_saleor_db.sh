...
if [ "$SALEOR_POSTGRES_INIT" == 'true' ]; then
	psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
		CREATE USER $SALEOR_POSTGRES_USER WITH PASSWORD '$SALEOR_POSTGRES_PASSWORD';
		CREATE DATABASE $SALEOR_POSTGRES_DB;
		GRANT ALL PRIVILEGES ON DATABASE $SALEOR_POSTGRES_DB TO $SALEOR_POSTGRES_USER;
		ALTER ROLE $SALEOR_POSTGRES_USER CREATEROLE SUPERUSER;
	EOSQL
	echo
fi
