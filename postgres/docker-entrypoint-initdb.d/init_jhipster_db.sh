...
if [ "$JHIPSTER_POSTGRES_INIT" == 'true' ]; then
	psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
		CREATE USER $JHIPSTER_POSTGRES_USER WITH PASSWORD '$JHIPSTER_POSTGRES_PASSWORD';
		CREATE DATABASE $JHIPSTER_POSTGRES_DB;
		GRANT ALL PRIVILEGES ON DATABASE $JHIPSTER_POSTGRES_DB TO $JHIPSTER_POSTGRES_USER;
		ALTER ROLE $JHIPSTER_POSTGRES_USER CREATEROLE SUPERUSER;
	EOSQL
	echo
fi
