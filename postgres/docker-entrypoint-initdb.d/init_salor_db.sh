...
if [ "$SALOR_POSTGRES_INIT" == 'true' ]; then
	psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
		CREATE USER $SALOR_POSTGRES_USER WITH PASSWORD '$SALOR_POSTGRES_PASSWORD';
		CREATE DATABASE $SALOR_POSTGRES_DB;
		GRANT ALL PRIVILEGES ON DATABASE $SALOR_POSTGRES_DB TO $SALOR_POSTGRES_USER;
		ALTER ROLE $SALOR_POSTGRES_USER CREATEROLE SUPERUSER;
	EOSQL
	echo
fi
