...
if [ "$MLFLOW_POSTGRES_INIT" == 'true' ]; then
	psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
		CREATE USER $MLFLOW_POSTGRES_USER WITH PASSWORD '$MLFLOW_POSTGRES_PASSWORD';
		CREATE DATABASE $MLFLOW_POSTGRES_DB;
		GRANT ALL PRIVILEGES ON DATABASE $MLFLOW_POSTGRES_DB TO $MLFLOW_POSTGRES_USER;
		ALTER ROLE $MLFLOW_POSTGRES_USER CREATEROLE SUPERUSER;
	EOSQL
	echo
fi
